...
class Vortex_Checkout_Api_Clickandcollect_Clear_Put implements Vortex_Api_EndpointInterface
{
    ...
    public function execute($body, Zend_Controller_Request_Http $request, Zend_Controller_Response_Http $response)
    {
         ...
         * memory.
         */
        $newBasket = Mage::getModel('sales/quote')
            ->load($this->getBasket()->getId());

        // This is necessary to establish a new shipping address once the C+C address is cleared
        $newBasket->getShippingAddress();

        $newBasket->setTotalsCollectedFlag(false)
            ->collectTotals();
